...
async function getEbayApplicationAccessToken() {
  if (
    ebayAppTokenCache.accessToken &&
    ebayAppTokenCache.expiresAtMs > nowMs() + 60 * 1000
  ) {
  ...
  ebayAppTokenCache.accessToken = response.access_token;
  ebayAppTokenCache.expiresAtMs =
    nowMs() + (Number(response.expires_in || 7200) - 60) * 1000;
